<template>
  <div class="m-0 mt-1">
    ...
    <div class="d-flex pills">
      <div v-for="(value, key) in pillList" v-bind:key="key" class="pill rounded-pill p-1 px-2 me-1 mb-1">
        <p> {{ value }} </p>
      </div>
      <div v-if="pillList.length==0" class="pill rounded-pill p-1 px-2 mb-1 me-1">
        <p> None </p>
      </div>
    </div>
  </div>
